...
// Parse FormData items(converting into array)
function parseItemsField(val) {
  ...
  if (typeof val === "string") {
    try {
      return JSON.parse(val); //string → array
    } catch (error) {
      return [];
    }
  }
...
function uploadedFilesToUrls(req) {
  ...
  return urls;
  // { logoDataUrl: "http://localhost:3000/uploads/logo.png" }
}

// It creates a unique invoice number and makes sure it doesn’t already exist in DB
async function generateUniqueInvoiceNumber(attempts = 8) {
  ...
  return new mongoose.Types.ObjectId().toString(); //or generates a unique ID string using MongoDB’s ObjectId
}
...
export async function createInvoice(req, res) {
  try {
    ...
    // If client supplied invoiceNumber, ensure it doesn't already exists
    ...
    const doc = new Invoice({
      ...
      status: ["draft", "paid", "unpaid", "overdue"].includes(body.status)
        ? body.status
        : "draft",
    });
...
export async function updateInvoice(req, res) {
  try {
    ...
    // If user changes the invoice number, then ensure that it does not exists already
    if (
      body.invoiceNumber &&
    ...
    const update = {
      ...
      aiConfidence: body.aiConfidence ?? existing.aiConfidence,
...
export async function deleteInvoice(req, res) {
  try {
    ...
    const query = isObjectIdString(id)
      ? { _id: id, owner: userId }
      : { invoiceNumber: id, owner: userId };
    const found = await Invoice.findOneAndDelete(query);
